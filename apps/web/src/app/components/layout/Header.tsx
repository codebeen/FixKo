...
export default function Header({ onBurgerClick, opened }: TopHeaderProps) {
    ...
    const currentNav = NAV_CONFIG.find((item) =>
        pathname?.startsWith(`/${item.link}`),
    );

    const breadcrumbData = [{ title: "FixKoPH", href: `/dashboard` }];

    if (currentNav && currentNav.link !== "dashboard") {
        breadcrumbData.push({
            title: currentNav.label,
            href: `/${currentNav.link}`,
        });
    }
    ...
    return (
        ...
        >
            <Group
                ...
                w="100%"
                px={{ base: "sm", md: 40 } as any}
                wrap="nowrap"
            >
                {/* Left Section */}
                <Group
                    gap={{ base: "xs", sm: "md", md: "lg" } as any}
                    wrap="nowrap"
                >
                    {isMobile && (
                        <Burger
                ...
                {/* Right Section */}
                <Group
                    gap={{ base: "sm", sm: "md", md: "xl" } as any}
                    wrap="nowrap"
                >
                    <Notification />
