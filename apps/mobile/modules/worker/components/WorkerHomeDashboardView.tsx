...
export default function WorkerHomeDashboardView() {
  ...
  // Simulated job leads
  const jobLeads = [
    { id: '1', title: 'Garden Cleaning', distance: '1.2 km away', payout: '₱450', client: 'Toby Smith' },
    { id: '2', title: 'Kitchen Sink Plumbing', distance: '2.5 km away', payout: '₱900', client: 'Mary Jane' },
  ];
  ...
  return (
    <BaseMain>
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 60 }} className="flex-1 px-5 pt-4">
        ...
        <TouchableOpacity 
          onPress={() => router.push('/(worker)/booking/JobOverview' as any)}
          className="bg-brand-blue/15 border border-brand-blue/30 rounded-2xl p-5 mb-6"
        >
          <View className="flex-row justify-between items-start mb-3">
            <View>
              <Text className="text-white text-base font-bold">Darben Client</Text>
              <Text className="text-[#7EB1F1] text-xs font-semibold">House Deep Cleaning</Text>
              <Text className="text-brand-grey-light text-[11px] mt-1">Address: Brgy. Burgos, Rodriguez, Rizal</Text>
            </View>
            <View className="bg-brand-green/20 px-2 py-0.5 rounded-full border border-brand-green/30">
              <Text className="text-brand-green text-[9px] font-bold">ACTIVE</Text>
            </View>
          </View>
          <View className="h-[1px] bg-white/10 my-3" />
          <View className="flex-row justify-between items-center">
            <Text className="text-white/60 text-xs">Payout: ₱700 (GCash)</Text>
            <View className="bg-brand-blue px-4.5 py-1.5 rounded-full">
        ...
        {jobLeads.map(lead => (
          ...
          >
            <View className="flex-1 mr-4">
              ...
              <Text className="text-brand-yellow text-xs font-semibold mt-1">Payout: {lead.payout}</Text>
            </View>
            <TouchableOpacity 
              onPress={() => router.push('/(worker)/(tabs)/bookings' as any)}
