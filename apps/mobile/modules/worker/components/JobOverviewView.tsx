...
export default function JobOverviewView() {
    ...
    return (
        ...
        >
            ...
            >
                {/* Header */}
                <View className="flex-row justify-between items-center my-6 w-full">
                    <TouchableOpacity 
                    ...
                    <Text className="text-white text-[22px] font-bold text-center flex-1">Job Overview</Text>
                    <View style={{ width: 40 }} />
                </View>
                ...
                {client && (
                    <View className="w-full mb-5 bg-white/8 rounded-xl p-4 border border-white/15 flex-row justify-between items-center">
                        <View className="flex-1">
                            <Text className="text-[#66EE66] text-[11px] font-bold tracking-[1.5px] mb-1">CLIENT</Text>
                            <Text className="text-white text-[22px] font-bold">{client}</Text>
                        </View>
                    </View>
                )}
                ...
                {/* Map Section */}
                <View className="w-full rounded-[15px] overflow-hidden h-[180px] mb-6 border border-white/15">
                    <View className="flex-1 bg-[#1E2D4A] justify-center items-center">
                        <Ionicons name="location" size={24} color="#66EE66" className="mb-2" />
                        <Text className="text-white opacity-50 text-[13px]">Map Preview</Text>
                    </View>
                </View>
            </ScrollView>
            ...
            {status !== 'completed' && (
                <View className="w-full px-5 py-3.75 bg-[#001851] items-center justify-center">
                    <TouchableOpacity
                        className="bg-[#66EE66] rounded-[25px] py-3.75 items-center justify-center shadow-md"
                        style={{ width: width > 500 ? '60%' : '85%' }}
                        onPress={handleAcceptBooking}
                        activeOpacity={0.8}
                    >
                        <Text className="text-[#001540] font-bold text-base">Accept Booking</Text>
                    </TouchableOpacity>
                </View>
            )}
            ...
            <BaseModal
                ...
                overlayClassName="justify-start items-end pt-[60px] pr-5 bg-black/45"
                cardClassName="bg-brand-navy-dark rounded-2xl border border-white/15 w-[320px] max-w-full px-5 py-[18px] shadow-2xl"
            >
